...
export default function Match3Lobby({ onCreateRoom, onJoinRoom, onBack, username }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(15px)', padding: '2rem 25px', borderRadius: '20px', boxShadow: '0 15px 35px rgba(0,0,0,0.2)', width: '100%', maxWidth: '400px', textAlign: 'center', color: '#1e293b', border: '2px solid rgba(255,255,255,0.5)', position: 'relative' }}>
      ...
      {onBack && <button onClick={onBack} style={{ position: 'absolute', top: '15px', left: '15px', background: 'rgba(255,255,255,0.4)', color: '#0f766e', border: 'none', padding: '6px 12px', borderRadius: '10px', fontWeight: 'bold', fontSize: '1rem', cursor: 'pointer' }}>← 返回</button>}

      <h1 style={{ color: '#fff', textShadow: '0 2px 5px rgba(0,0,0,0.3)', margin: '20px 0 10px 0', fontSize: '2.5rem'}}>🍉 夏日消消樂</h1>
      ...
      <div style={{ background: 'rgba(255,255,255,0.5)', padding: '15px', borderRadius: '15px', marginBottom: '15px' }}>
        ...
        <button onClick={() => onCreateRoom(nickname || '遊客')} style={{ width: '100%', padding: '12px', marginTop: '5px', background: 'linear-gradient(135deg, #0ea5e9, #0284c7)', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer', fontSize: '1rem', boxShadow: '0 4px 10px rgba(14, 165, 233, 0.4)' }}>
          ➕ 建立房間
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '10px'}}>
        ...
        <button onClick={() => joinId && onJoinRoom(joinId, nickname || '遊客')} style={{ padding: '10px 20px', background: '#10b981', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold',fontSize: '1rem', cursor: 'pointer' }}>加入</button>
      </div>
