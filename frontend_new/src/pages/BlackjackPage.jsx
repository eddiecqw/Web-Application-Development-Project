...
export default function BlackjackPage({ user }) {
  ...
  const [glow, setGlow] = useState(false); // 控制開始按鈕的發光動畫
  const EMOJI_LIST = ['😎', '😭', '🤡', '💸', '😀', '😡', '💩', '🎉'];
    ...
    if (isPlaceholder) {
      return (
        <div key={`ph-${idx}`} style={{
          ...
          border: '2px dashed rgba(255,255,255,0.3)', backgroundColor: 'rgba(0,0,0,0.2)',
          position: 'relative'
        }} />
      );
    }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#1a4f2c', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px' }}>
      ...
      <header style={{ width: '100%', maxWidth: '900px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', backgroundColor: 'rgba(0,0,0,0.8)', borderRadius: '8px', marginBottom: '10px' }}>
        ...
        <div style={{ fontWeight: 'bold', color: '#ffd700', fontSize: '1.2rem' }}>房間: {roomId}</div>
      ...
      <div style={{ width: '100%', maxWidth: '900px', flex: 1, background: 'radial-gradient(circle, #226b3a 0%, #11361c 100%)', border: '10px solid #4a2e15', borderRadius: '20px', padding: '20px', boxShadow: 'inset 0 0 50px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        ...
        {/* 1. 莊家區域 */}
        <div style={{ textAlign: 'center', color: 'white', minHeight: '150px' }}>
          {isPlaying && (
            <>
              <h3 style={{ margin: '0 0 10px 0' }}>👑 莊家</h3>
          ...
          {!isPlaying && roomData?.status === 'waiting' && isOwner && (
            <div style={{ marginTop: '40px' }}>
              <button 
                onClick={() => startGame()} 
                style={{ 
                  padding: '12px 40px', background: 'linear-gradient(to bottom, #fbc02d, #f57f17)', 
                  color: '#3e2723', border: 'none', borderRadius: '25px', fontSize: '1.2rem', 
                  fontWeight: 'bold', cursor: 'pointer',
                  boxShadow: glow ? '0 0 20px rgba(255, 215, 0, 0.8)' : '0 4px 6px rgba(0,0,0,0.3)',
                  transition: 'box-shadow 0.4s ease-in-out'
                }}
              >
                🎮 開始發牌
              </button>
              <div style={{ color: '#ffd700', marginTop: '10px', fontSize: '0.9rem' }}>點擊開始為所有玩家發牌</div>
            </div>
          )}
          {!isPlaying && roomData?.status === 'waiting' && !isOwner && (
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', flexWrap: 'wrap', minHeight: '120px' }}>
          {opponents.map((opp) => (
            <div key={opp.name} style={{ textAlign: 'center', color: 'white', position: 'relative', background: roomData?.turn === opp.name ? 'rgba(255,215,0,0.1)' : 'transparent', padding: '10px', borderRadius: '10px' }}>
              {renderEmojiBubble(opp.name)}
              <h4 style={{ margin: '0 0 5px 0' }}>{opp.name.split('@')[0]} 💰 {opp.chips}</h4>
              <div style={{ display: 'flex', justifyContent: 'center', paddingLeft: '15px' }}>
                {/* ✨ 遊戲未開始時，顯示空牌框 */}
                {roomData.status === 'waiting' 
                  ? [0, 1].map(idx => renderCard(null, idx, true))
                  : opp.hand?.map((card, idx) => renderCard(card, idx))
                }
              </div>
              {roomData.status === 'showdown' && opp.result ? (
                <div style={{ color: opp.scoreChange > 0 ? '#00e676' : '#ff1744', fontWeight: 'bold', marginTop: '5px' }}>
                  {opp.result} ({opp.scoreChange > 0 ? `+${opp.scoreChange}` : opp.scoreChange})
                </div>
              ) : (
                <div style={{ color: '#ccc', fontSize: '0.9rem', marginTop: '5px' }}>{opp.score > 0 ? `${opp.score} 點` : '座位準備中'}</div>
              )}
            </div>
          ))}
        </div>
        ...
        {/* 3. 玩家自己的區域 */}
        {me && (
          <div style={{ textAlign: 'center', color: 'white', background: 'rgba(0,0,0,0.4)', padding: '15px', borderRadius: '15px', border: isMyTurn ? '3px solid #ffd700' : '2px solid transparent', position: 'relative' }}>
            {renderEmojiBubble(me.name)}
            
            <h2 style={{ margin: '0 0 10px 0', color: '#ffd700' }}>你的手牌 💰 {me.chips}</h2>
            <div style={{ display: 'flex', justifyContent: 'center', paddingLeft: '15px', minHeight: '100px' }}>
              {/* ✨ 遊戲未開始時，顯示空牌框 */}
              {roomData.status === 'waiting' 
                ? [0, 1].map(idx => renderCard(null, idx, true))
                : me.hand?.map((card, idx) => renderCard(card, idx))
              }
            </div>
            
            {roomData.status === 'showdown' && me.result ? (
              <div style={{ background: 'rgba(0,0,0,0.8)', color: me.scoreChange > 0 ? '#00e676' : '#ff1744', padding: '6px 16px', borderRadius: '20px', border: `2px solid ${me.scoreChange > 0 ? '#00e676' : '#ff1744'}`, fontSize: '1.2rem', fontWeight: 'bold', marginTop: '10px', display: 'inline-block' }}>
                {me.result} {me.scoreChange !== 0 && `(${me.scoreChange > 0 ? '+' : ''}${me.scoreChange})`}
              </div>
            ) : (
              <div style={{ background: 'rgba(0,0,0,0.7)', color: me.score > 21 ? '#ff1744' : '#00e676', padding: '4px 12px', borderRadius: '15px', border: `1px solid ${me.score > 21 ? '#ff1744' : '#00e676'}`, fontSize: '1rem', marginTop: '10px', display: 'inline-block' }}>
                目前點數：{me.score > 0 ? me.score : 0} {me.score > 21 && '(爆牌)'}
              </div>
            )}
            ...
            {isMyTurn && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '15px', flexWrap: 'wrap' }}>
                ...
                <button disabled style={{ background: '#9e9e9e', color: '#666', padding: '10px 20px', borderRadius: '25px', border: 'none', fontWeight: 'bold', fontSize: '1rem', cursor: 'not-allowed' }}>💰 加倍 (Double)</button>
                <button disabled style={{ background: '#9e9e9e', color: '#666', padding: '10px 20px', borderRadius: '25px', border: 'none', fontWeight: 'bold', fontSize: '1rem', cursor: 'not-allowed' }}>✂️ 分牌 (Split)</button>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ background: '#1a4f2c', border: '4px solid #ffd700', borderRadius: '15px', padding: '25px', maxWidth: '500px', width: '90%', color: 'white', position: 'relative', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            <div style={{ lineHeight: 1.6 }}>
              ...
              <p>莊家未滿 <strong>17點</strong> 必須要牌；達到 <strong>17點(含)以上</strong> 必須停牌。</p>
